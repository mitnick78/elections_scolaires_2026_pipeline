...
import type { Commune, Stats, Department } from "../types";
...
export const getDepartements = async (): Promise<Department[]> => {
  const { data } = await api.get("/departements");
  return data;
};

// Communes par département
export const getCommunesByDepartement = async (
  departement: string,
): Promise<Commune[]> => {
...
export const getCommunesByTension = async (
  niveau: string,
): Promise<Commune[]> => {
